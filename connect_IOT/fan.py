...
class NeuralNetwork(nn.Module):
    def __init__(self):
        ...
        list_label = label_dict_from_config_file("./connect_IOT/hand_gesture.yaml")
...
def main():
    ...
    LABEL_TAG = label_dict_from_config_file("./connect_IOT/hand_gesture.yaml")
    LABEL_NAMES = [name for _, name in LABEL_TAG.items()]

    # Tải mô hình đã huấn luyện và chuyển sang thiết bị đã chọn
    model = NeuralNetwork().to(device)
    model.load_state_dict(torch.load('connect_IOT/model/model_Hand_Gesture_MLP.pth', map_location=device))
    model.eval()
    ...
    while cam.isOpened():
        ...
        if hands:
            ...
            if (old_predict == None or predict != old_predict) and predict != 4:
                client.publish(feed_id, predict)
                old_predict = predict
