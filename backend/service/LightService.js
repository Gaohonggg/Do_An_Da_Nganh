        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (status != rac1[rac1.length - 1].status) {
                    const [rac4] = await db.query('INSERT INTO control (CCCD, device_type, device_id, status, date, hour) VALUES (?, ?, ?, ?, ?, ?)', [req.session.user.id, 'light', id, status, support.getDay(), support.getHour()]);
