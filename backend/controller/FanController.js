...
class FanController {
    ...
    setFan = async (req, res) => {
        try {
            ...
            var status = "";
            if (level == null || level < 0 || level > 3) return res.status(404).json({status: false, message: "Thiếu thông tin điều khiển"});
            if (level == 0) status = "OFF";
            else if (level > 0) status = "ON";
            const result = await FanService.setFan({ id, status, level }, req);
            return res.status(200).send(result);
        } catch(err) {
            return res.status(404).json({status: false, error: err});
        }
